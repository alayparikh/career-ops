# Mode: batch — Batch Processing of Job Offers
...
Claude Conductor (claude --chrome --dangerously-skip-permissions)
  ...
  │
  ├─ Job Offer 1: reads JD from DOM + URL
  │    └─► claude -p worker → report .md + PDF + tracker-line
  │
  ├─ Job Offer 2: click next, reads JD + URL
  │    └─► claude -p worker → report .md + PDF + tracker-line
  │
...
- If it dies → re-execute → read `batch-state.tsv` → skip completed ones
- Lock file (`batch-runner.pid`) prevents double execution
- Each worker is independent: failure in offer #47 does not affect others
...
| Conductor dies | Re-execute → read state → skip completed |
